...
void fetch_weather()
{
    ...
    char url[100];
    sprintf(url,"https://weatherapi-com.p.rapidapi.com/forecast.json?q=Melbourne&days=%d",NUM_DAYS);
    ...
    if (err == ESP_OK)
    {
        ...
        weather_t weather;
        parse_weather(&weather,(char *)chunk_payload.buffer);
...
void app_main(void)
{
    ...
    wifi_init();
    xTaskCreate(weather_display_task,"weather_display_task",1024 *5,NULL,10,NULL);
