...
esp_err_t send_web_socket_message(char *message)
{
  ...
  return httpd_ws_send_frame_async(server, client_session_id, &ws_responce);
}
...
static esp_err_t on_web_socket_url(httpd_req_t *req)
{
  ...
  ws_pkt.payload = malloc(MAX_LEN);
  httpd_ws_recv_frame(req, &ws_pkt, MAX_LEN);
  printf("payload: %.*s\n", ws_pkt.len, ws_pkt.payload);

  char *responce = "connected ok 😁";
  httpd_ws_frame_t ws_responce = {
      .final = true,
      .fragmented = false,
      .len = strlen(responce),
      .payload = (uint8_t *)responce,
      .type = HTTPD_WS_TYPE_TEXT};

  httpd_ws_send_frame(req, &ws_responce);
